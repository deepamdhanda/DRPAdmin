...
const TicketScreen: React.FC = () => {
  ...
  const filteredTickets = useMemo(() => {
    ...
    if (filtered.length > 0) {
      filtered = filtered.sort(
        (a, b) =>
          new Date(a.slaDueBy).getTime() - new Date(b.slaDueBy).getTime()
      );
    }
    return filtered;
  ...
  const columns: TableColumn<Ticket>[] = [
      ...
      cell: (row) => (
        <>
          <div>
            ...
            {row.isEscalated && (
              <span style={{ color: "red", fontWeight: "bold", marginLeft: 6 }}>
                🚩 Escalated
              </span>
            )}
          </div>
    ...
    },
    {
      ...
      cell: (row) => (
        <>
          <small>
            <span style={{ color: STATUS_COLORS[row.status] || "#000" }}>
              {row.status}
            </span>
          </small>
        </>
    ...
    },
    {
      ...
      cell: (row) => (
        <>
          <small>{new Date(row.slaDueBy).toLocaleString()}</small>
        </>
      ),
    },
    {
      ...
      cell: (row) => (
        <>
          <div>
            <strong>{row.createdBy?.name ?? "System"}</strong>
          </div>
        </>
      ),
    },
  ...
  const handleBulkClose = async () => {
    ...
    try {
      await axios.post("/api/tickets/bulk-close", {
        ids: selectedTickets.map((t) => t._id),
      });
