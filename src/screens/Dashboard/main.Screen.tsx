...
const menuItems = [
  ...
  {
    name: 'Marketing', icon: '', path: "/marketing", child: [
      { name: 'Create Automation', icon: '➕', path: "/marketing/createAutomation" },
      ...
      { name: 'Whatsapp Message Template', icon: '➕', path: "/marketing/template/whatsappMessage" },
    ]
  },
  {
    name: 'Whatsapp', icon: '', path: "/whatsapp", child: [
...
const renderMenu = (
  ...
  return (
    <ul style={{ listStyle: 'none', paddingLeft: level === 0 ? 0 : '1.5rem', margin: 0 }}>
        ...
        return (
          <li key={name}>
            <div
              ...
              onMouseLeave={e =>
                (e.currentTarget.style.backgroundColor = isActive ? '#f5891e' : 'transparent')
              }
              onFocus={e =>
                (e.currentTarget.style.backgroundColor = '#f5891e')
              }
            ...
            {child && renderMenu(child, activeLink, setActiveLink, navigate, setMenuOpen, level + 1)}
          </li>
        );
      })}
    </ul>
      ...
      >
        ...
        {menuOpen && (<nav
          ...
          style={{
            width: sidebarWidth,
            background:
              'linear-gradient(180deg, #000434 0%, #0d1446 100%)',
            color: '#fff',
            padding: '0.6rem',
            position: 'fixed',
            top: 0,
        ...
        >
          ...
          >
            {renderMenu(menuItems, activeLink, setActiveLink, navigate, setMenuOpen)}
        ...
        <main
          style={{
            ...
            backgroundColor: '#fff',
            boxShadow: menuOpen
              ? '-8px 0 15px rgba(0, 0, 0, 0.1)'
              : 'none',
            transition: 'margin-left 0.35s cubic-bezier(0.4,0,0.2,1), box-shadow 0.35s ease',
          }}
        >
          ...
          >
            ...
            <div style={{ marginLeft: 20, flexDirection: "column", alignItems: "baseline", display: "flex" }}>
              {/* Title or logo can go here */}
              <h1 style={{ fontWeight: 'normal', fontSize: '1.25rem', marginBottom: 0 }}>
                {pageName}
              </h1>
